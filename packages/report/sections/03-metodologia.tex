...
\begin{itemize}
  \item $X=(0,0,0,0,0)$: $y^*=81.72$, etiqueta \emph{alto}. Activan con $\alpha=1$ nueve reglas (R1, R2, R3, R10, R16, R17, R18, R19 y R20), que apuntan a riesgo crítico y alto; el centroide se ubica en la frontera entre alto y crítico porque la agregación máximo combina ambos conjuntos consecuentes.
  \item $X=(100,100,100,100,100)$: $y^*=13.83$, etiqueta \emph{bajo}. Activan con $\alpha=1$ cuatro reglas (R5, R6, R9 y R15), todas con consecuente \emph{bajo}. El centroide cae dentro del conjunto $B_{\text{bajo}}$ y la masa agregada se concentra en $[0,35]$.
  \item $X=(50,50,50,50,50)$: $y^*=71.73$, etiqueta \emph{alto}. Caso aparentemente neutral pero las funciones de pertenencia están calibradas con criterio experto, no de forma simétrica: en $x=50$ los conjuntos \emph{baja asistencia}, \emph{entregas insuficientes} y \emph{deficientes} todavía valen $\mu>0.5$, lo que dispara las reglas críticas R1, R2, R3, R16 y R17 con $\alpha=0.67$, complementadas por reglas intermedias (R14, R22, R24, R26) con $\alpha=0.25$. La masa resultante se concentra en la zona alto-crítico. Este efecto es deseado: refleja que un perfil mediocre en cinco dimensiones simultáneas no es "promedio" sino "preocupante".
\end{itemize}
